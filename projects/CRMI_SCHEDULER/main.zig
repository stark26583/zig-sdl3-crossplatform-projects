...
const AppState = struct {
    ...
    var renderer_name: [:0]const u8 = undefined;
...
fn init(
    ...
    const before_window = sdl3.timer.getMillisecondsSinceInit();
    ...
    const afwbr = sdl3.timer.getMillisecondsSinceInit();

    const renderer = try sdl3.render.Renderer.init(window, null);
    errdefer renderer.deinit();

    const afr = sdl3.timer.getMillisecondsSinceInit();

    try sdl3.log.log("before window: {d}", .{before_window});
    try sdl3.log.log("after window before_renderer: {d}", .{afwbr});
    try sdl3.log.log("after renderer and window: {d}", .{afr});

    try sdl3.log.log("between create window: {d}", .{afwbr - before_window});
    try sdl3.log.log("between renderer: {d}", .{afr - afwbr});

    AppState.renderer_name = try renderer.getName();
...
fn iterate(
    ...
    //Draw GUI
    zgui.backend.newFrame(@intCast(size.width), @intCast(size.height));

    zgui.backend.draw(state.renderer.value);
...
fn event(
    ...
    _ = state;
    var eve = curr_event;
    _ = zgui.backend.processEvent(eve.toSdlInPlace());
