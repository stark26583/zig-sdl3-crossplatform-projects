...
pub fn build(b: *std.Build) !void {
    ...
    // const zgui_enabled = b.option(bool, "zgui", "Enable zgui") orelse false;
    // extension_options.addOption(bool, "zgui", zgui_enabled);
    // TODO:
    //
    ...
    for (projects) |project| {
        ...
        for (targets) |target| {
            ...
            //         if (android_apk) |apk| {
            //             zgui_mod.addSystemIncludePath(.{ .cwd_relative = apk.ndk.sysroot_path });
            //             const android_triple = try getAndroidTriple(target);
            //             zgui_mod.addLibraryPath(.{ .cwd_relative = b.pathJoin(&.{ apk.ndk.sysroot_path, "usr", "lib", android_triple, b.fmt("{}", .{@intFromEnum(android_version)}) }) });
            //
            //             zgui_mod.addIncludePath(sdl_dep.path("include"));
            //             zgui_lib.addSystemIncludePath(.{ .cwd_relative = apk.ndk.sysroot_path });
            //             zgui_lib.addLibraryPath(.{ .cwd_relative = b.pathJoin(&.{ apk.ndk.sysroot_path, "usr", "lib", android_triple, b.fmt("{}", .{@intFromEnum(android_version)}) }) });
            //             const lib_c = apk.sdk.createOrGetLibCFile(zgui_lib, .android15, apk.ndk.sysroot_path, apk.ndk.version);
            //             zgui_lib.setLibCFile(lib_c);
            //         }
            //
            //         cat_module.addImport("zgui", zgui_mod);
            //
            //         break :blk zgui_lib;
            //     } else break :blk null;
            // };
    ...
    for (gpu_projects) |project| {
        ...
        for (targets) |target| {
            ...
            const zgui_gpu_lib: *std.Build.Step.Compile = blk: {
                ...
                cat_module.addImport("zgui", zgui_gpu_mod);
...
fn compileHlslShaders(b: *std.Build, module: *std.Build.Module, run_step: *std.Build.Step, src_dir_path: []const u8, out_dir_path: []const u8) !void {
    ...
    while (try shader_dir_iter.next()) |shader| {
        if (shader.kind != .file or !(std.mem.endsWith(u8, shader.name, ".hlsl")))
            continue;
        compileHlslShader(b, module, run_step, src_dir_path, shader.name, out_dir_path, b.fmt("{s}.spirv", .{shader.name[0 .. shader.name.len - 5]}));
    }
